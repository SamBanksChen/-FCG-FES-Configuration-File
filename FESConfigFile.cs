...
namespace FESConfigFile
{
    public class FCG_Document
    {
        ...
        public bool Open(string Filepath)
        {
            ...
            {
                AddWarning(0, 0, "Version:Version are not compatible");
                return false;
            }
            index = vinfo.IndexOf('D');
            LightDataVersion = new byte[2] { byte.Parse(vinfo[index + 1].ToString()), byte.Parse(vinfo[index + 2].ToString()) };
            if (LightDataVersion[0] != FCG_LightCfg.SyntaxVersion && LightDataVersion[1] > FCG_LightCfg.StableVersion)
            {
                AddWarning(0, 0, "Version:Data are not compatible");
                return false;
            }
        ...
        public FCG_Document Save(string filepath)
        {
            Filepath = filepath;
            File.WriteAllText(Filepath,ToString());
            return this;
        ...
        public bool PathGetValue(string path,out string value)
        {
            ...
            if(pathnodes.Count > 0 && pathnodes[i] == now.Name)
            {
                i++;
    ...
    public class FCG_Node : IDisposable
    {
        string value;
        string name;
        public FCG_Document ParentDocument;
        public FCG_Node ParentNode;
        public FCG_LightCfg LightData;
        bool IsLightData = false;
        private Dictionary<string, FCG_Node> ChildNodes = new Dictionary<string, FCG_Node>();
        ...
        public bool ValueAsLightCfg()
        {
            LightData = new FCG_LightCfg();
            if(LightData.Parse(ParentDocument.LightDataVersion[0], ParentDocument.LightDataVersion[1], value))
            {
                IsLightData = true;
                return true;
            }
        ...
        public FCG_Node SetLightCfg(FCG_LightCfg lightCfg)
        {
            IsLightData = true;
            LightData = lightCfg;
            return this;
        ...
        public bool AddNode(FCG_Node node)
        {
            if(!ChildNodes.ContainsKey(node.Name))
            {
                ...
                node.ParentDocument = ParentDocument;
                return true;
            }
            return false;
        }
        ...
        public FCG_Node RemoveNode(string name)
        {
            if (ChildNodes.ContainsKey(name))
            {
                ...
                ChildNodes[name].ParentDocument = null;
        ...
        public override string ToString()
        {
            ...
            if (IsLightData)
            {
                var l = LightData.ToLines();
                var sbb = new StringBuilder();
                foreach (string s in l)
                {
                    sbb.Append(s).Replace("\\", "\\\\").Replace("<", "\\<").Replace(">", "\\>");
                    sb.AppendLine().Append(sbb.ToString());
                    sbb.Clear();
                }
            }
            else if(value != "")
            {
                var vb = new StringBuilder(value);
                vb.Replace("\\", "\\\\").Replace("<", "\\<").Replace(">", "\\>");
                sb.Append(vb);
            }
        ...
        public StringBuilder ToString(int tab, bool TabToFourSpace)
        {
            ...
            if (IsLightData)
            {
                ...
                foreach (string s in l)
                {
                    sbb.Append(Tab(tab + 1,TabToFourSpace)).Append(s).Replace("\\", "\\\\").Replace("<", "\\<").Replace(">", "\\>");
                    sb.AppendLine().Append(sbb.ToString());
                    sbb.Clear();
                }
            }
            else if (value != "")
            {
                var vb = new StringBuilder(value);
                vb.Replace("\\", "\\\\").Replace("<", "\\<").Replace(">", "\\>");
                sb.AppendLine().Append(Tab(tab + 1, TabToFourSpace)).Append(vb);
            }
